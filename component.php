...
function getImagePath($src, $check = false)
{
    ...
    if (!$imageInfo)
        $errors .= GetMessage("IMG_NOT_EXIST");
    elseif ($check)
    {
        if ($check["WIDTH"] && $check["WIDTH"] != $imageInfo[0])
            $errors .= GetMessage("IMG_WIDTH") . ($check["WIDTH"] > $imageInfo[0] ? GetMessage("IMG_WIDTH_LARGER") : GetMessage("IMG_WIDTH_LESS")) .  $check["WIDTH"] . GetMessage("IMG_PX");
        if ($check["HEIGHT"] && $check["HEIGHT"] != $imageInfo[1])
            $errors .= GetMessage("IMG_HEIGHT") . ($check["HEIGHT"] > $imageInfo[1] ? GetMessage("IMG_HEIGHT_LARGER") : GetMessage("IMG_HEIGHT_LESS")) . $check["WIDTH"] . GetMessage("IMG_PX");
        if ($check["MIME"] && !in_array($imageInfo["mime"], $check["MIME"]))
        {
            $errors .= GetMessage("IMG_MIME");
